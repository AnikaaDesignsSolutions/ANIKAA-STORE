...
const CartDropdown = ({
  ...
  useEffect(() => {
    if (itemRef.current !== totalItems && !pathname.includes("/explore/cart")) {
      timedOpen()
    }
    ...
    >
      <Popover className="relative h-full">
        ...
        <LocalizedClientLink
          ...
          >
            <FontAwesomeIcon icon={faShoppingCart} className="text-xl" style={{color:"black"}}/>
            {totalItems > 0 && (
              <span className="absolute top-0 right-0 transform translate-x-1/2 -translate-y-1/2 bg-pink-500 text-white text-xs font-semibold rounded-full h-5 w-5 flex items-center justify-center" style={{background:"#EE0A67"}}>
                {totalItems}
              </span>
            )}
          </LocalizedClientLink>
                ...
                <div className="overflow-y-scroll max-h-[402px] px-4 grid grid-cols-1 gap-y-8 no-scrollbar p-px">
                  {cartState.items
                    ...
                    .map((item) => (
                      ...
                      >
                        <LocalizedClientLink
                          href={`/explore/products/${item.variant.product.handle}`}
                          className="w-24"
                        >
